...
| `LESS` | `LESS <filename>` | Pages through a file, forward AND backward, by screen or by line. `SPACE`/`F`/PgDn = next page, `B`/PgUp = previous page, `g` = go to the top, `G` = go to the end; Down-arrow/`J`/Ctrl-N/Ctrl-E = down one line, Up-arrow/`K`/Ctrl-P/Ctrl-Y = up one line (moving up or back past the point where `LESS` has any recorded history -- e.g. right after `G`, `g`, or a search match -- scans backward through the file for the true previous line/page instead of just stopping); `/` = search forward (case-sensitive), `N` = repeat the last search, `Q` = quit. A search pattern may contain escape sequences -- `\\n`, `\\r`, `\\t`, `\\0`, `\\\\` for a literal backslash, and `\\xHH` for any byte by hex value (exactly two digits) -- so you can search for tabs, line endings, or arbitrary binary bytes, and a pattern may span a line boundary. A backslash followed by anything else is reported as an error rather than searched for literally. |
